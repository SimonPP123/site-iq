...
// Returns null (N/A) when <2 non-empty values so an all-empty site (no titles / no meta) does NOT
// score a false 1.0 "all unique" PASS; C() treats null as N/A and renormalizes it out. Mirrors checks.ts.
const uniqRatio = (vals) => { const v = vals.map(x => (x || '').trim().toLowerCase()).filter(Boolean); if (v.length < 2) return null; const c = {}; for (const x of v) c[x] = (c[x] || 0) + 1; return v.filter(x => c[x] === 1).length / v.length; };
const hostOf = (u) => { const m = String(u || '').match(/^https?:\/\/([^/?#]+)/i); return (m ? m[1] : '').replace(/^www\./, '').toLowerCase(); };
...
const checks = [
  ...
  C('S23', 'Canonical resolves to this page (no cross-page mismatch)', 'seo', 5, 'medium',
    covR(p => { const m = src(p).match(/<link[^>]+rel=["']canonical["'][^>]*href=["']([^"']+)["']/i) || src(p).match(/<link[^>]+href=["']([^"']+)["'][^>]*rel=["']canonical["']/i); if (!m) return null; const self = mt(p).sourceURL || meta.rootUrl || ''; const origin = (self.match(/^https?:\/\/[^/]+/i) || [''])[0]; let canon = m[1].trim(); if (canon.startsWith('//')) canon = 'https:' + canon; else if (canon.startsWith('/')) canon = origin + canon; const norm = (u) => u.replace(/^https?:\/\//i, '').replace(/^www\./i, '').replace(/[#?].*$/, '').replace(/\/+$/, '').toLowerCase(); if (norm(canon) === norm(self)) return null; const canonPath = (canon.replace(/^https?:\/\/[^/]+/i, '').replace(/[#?].*$/, '') || '/').slice(0, 80); return { kind: 'mismatch', what: 'canonical', expected: pathOf(self), actual: canonPath }; }), { effort: 2 }),
  ...
  C('G6', 'Statistics & data points', 'geo', 8, 'medium',
    covR(p => { const m = md(p); const stats = (m.match(/\d[\d.,]*\s*(?:%|‰|percent)(?![a-z])/gi) || []).length + (m.match(/[€$£¥]\s?\d[\d.,]*/g) || []).length + (m.match(/\b(?:\d{2,}|\d+[.,]\d+)[\d.,]*\s*(?:k|m|bn|million|billion|thousand)(?![a-z])/gi) || []).length + (m.match(/\b\d+\s*(?:in|of|out of)\s*\d+\b/gi) || []).length; return stats >= 3 ? null : { kind: 'wrong_count', what: 'concrete statistics (%, currency, ratios)', actual: stats, expected: 3 }; }), { effort: 4 }),
